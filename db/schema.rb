...
ActiveRecord::Schema.define(:version => 20120430153657) do
  ...
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_aliquots WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_aliquots (SELECT * FROM aliquots WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_aliquots_trigger", :event=>:insert, :on=>"aliquots"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_asset_audits WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_asset_audits (SELECT * FROM asset_audits WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_asset_audits_trigger", :event=>:insert, :on=>"asset_audits"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_asset_links WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_asset_links (SELECT * FROM asset_links WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_asset_links_trigger", :event=>:insert, :on=>"asset_links"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_batch_requests WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_batch_requests (SELECT * FROM batch_requests WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_batch_requests_trigger", :event=>:insert, :on=>"batch_requests"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_batches WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_batches (SELECT * FROM batches WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_batches_trigger", :event=>:insert, :on=>"batches"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_billing_events WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_billing_events (SELECT * FROM billing_events WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_billing_events_trigger", :event=>:insert, :on=>"billing_events"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_events WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_events (SELECT * FROM events WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_events_trigger", :event=>:insert, :on=>"events"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_lanes WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_lanes (SELECT * FROM lanes WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_lanes_trigger", :event=>:insert, :on=>"lanes"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_library_tubes WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_library_tubes (SELECT * FROM library_tubes WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_library_tubes_trigger", :event=>:insert, :on=>"library_tubes"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_multiplexed_library_tubes WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_multiplexed_library_tubes (SELECT * FROM multiplexed_library_tubes WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_multiplexed_library_tubes_trigger", :event=>:insert, :on=>"multiplexed_library_tubes"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_orders WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_orders (SELECT * FROM orders WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_orders_trigger", :event=>:insert, :on=>"orders"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_plate_purposes WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_plate_purposes (SELECT * FROM plate_purposes WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_plate_purposes_trigger", :event=>:insert, :on=>"plate_purposes"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_plates WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_plates (SELECT * FROM plates WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_plates_trigger", :event=>:insert, :on=>"plates"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_projects WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_projects (SELECT * FROM projects WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_projects_trigger", :event=>:insert, :on=>"projects"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_quotas WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_quotas (SELECT * FROM quotas WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_quotas_trigger", :event=>:insert, :on=>"quotas"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_requests WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_requests (SELECT * FROM requests WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_requests_trigger", :event=>:insert, :on=>"requests"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_sample_tubes WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_sample_tubes (SELECT * FROM sample_tubes WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_sample_tubes_trigger", :event=>:insert, :on=>"sample_tubes"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_samples WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_samples (SELECT * FROM samples WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_samples_trigger", :event=>:insert, :on=>"samples"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_studies WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_studies (SELECT * FROM studies WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_studies_trigger", :event=>:insert, :on=>"studies"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_study_samples WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_study_samples (SELECT * FROM study_samples WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_study_samples_trigger", :event=>:insert, :on=>"study_samples"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_submissions WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_submissions (SELECT * FROM submissions WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_submissions_trigger", :event=>:insert, :on=>"submissions"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_tags WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_tags (SELECT * FROM tags WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_tags_trigger", :event=>:insert, :on=>"tags"})
  after_trigger("BEGIN\n            IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n              DELETE FROM current_wells WHERE uuid=NEW.uuid;\n            END ; END IF ;\n            IF NEW.current_to IS NULL THEN BEGIN\n              INSERT INTO current_wells (SELECT * FROM wells WHERE uuid=NEW.uuid AND current_from=NEW.current_from AND current_to IS NULL);\n            END ; END IF ;\n          END", {:name=>"maintain_current_wells_trigger", :event=>:insert, :on=>"wells"})
