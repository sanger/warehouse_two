...
ActiveRecord::Schema.define(:version => 20130724103504) do
  ...
  after_trigger("BEGIN\n          IF NEW.current_to IS NULL OR NEW.deleted_at IS NOT NULL THEN BEGIN\n            DELETE FROM current_studies WHERE uuid=NEW.uuid;\n          END ; END IF ;\n          IF NEW.current_to IS NULL THEN BEGIN\n            INSERT INTO current_studies(`uuid`,`internal_id`,`name`,`reference_genome`,`ethically_approved`,`faculty_sponsor`,`state`,`study_type`,`abstract`,`abbreviation`,`accession_number`,`description`,`is_current`,`checked_at`,`last_updated`,`created`,`contains_human_dna`,`contaminated_human_dna`,`data_release_strategy`,`data_release_sort_of_study`,`ena_project_id`,`study_title`,`study_visibility`,`ega_dac_accession_number`,`array_express_accession_number`,`ega_policy_accession_number`,`inserted_at`,`deleted_at`,`current_from`,`current_to`,`data_release_timing`,`data_release_delay_period`,`data_release_delay_reason`,`remove_x_and_autosomes`,`alignments_in_bam`,`seperate_y_chromosome_data`) VALUES(NEW.uuid,NEW.internal_id,NEW.name,NEW.reference_genome,NEW.ethically_approved,NEW.faculty_sponsor,NEW.state,NEW.study_type,NEW.abstract,NEW.abbreviation,NEW.accession_number,NEW.description,NEW.is_current,NEW.checked_at,NEW.last_updated,NEW.created,NEW.contains_human_dna,NEW.contaminated_human_dna,NEW.data_release_strategy,NEW.data_release_sort_of_study,NEW.ena_project_id,NEW.study_title,NEW.study_visibility,NEW.ega_dac_accession_number,NEW.array_express_accession_number,NEW.ega_policy_accession_number,NEW.inserted_at,NEW.deleted_at,NEW.current_from,NEW.current_to,NEW.data_release_timing,NEW.data_release_delay_period,NEW.data_release_delay_reason,NEW.remove_x_and_autosomes,NEW.alignments_in_bam,NEW.seperate_y_chromosome_data);\n          END ; END IF ;\n        END", {:name=>"maintain_current_studies_trigger", :event=>:insert, :on=>"studies"})
